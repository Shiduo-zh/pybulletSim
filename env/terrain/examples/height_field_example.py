...
#max height
heightPerturbationRange = 1
#create a 256×256 grid by orogramming,each element denotes a height data
if heightfieldSource==useProgrammatic:
  numHeightfieldRows = 768
  numHeightfieldColumns = 768
  #matrix initial: a 256*256 list
  ...
  #heightfield collision model set
  ...
  textureId=p.loadTexture("..//heightmaps//Textures.png")
  terrain  = p.createMultiBody(0, terrainShape)
  p.resetBasePositionAndOrientation(terrain,[0,0,0], [0,0,0,1])
  p.changeVisualShape(terrain,-1,textureUniqueId=textureId)

#create heightfield by locol csv file
if heightfieldSource==useDeepLocoCSV:
  terrainShape = p.createCollisionShape(shapeType = p.GEOM_HEIGHTFIELD, meshScale=[.5,.5,2.5],fileName = "..//heightmaps/sea.txt", heightfieldTextureScaling=128)
  terrain  = p.createMultiBody(0, terrainShape)
  p.resetBasePositionAndOrientation(terrain,[0,0,0], [0,0,0,1])

#create heightfield by locol picture
if heightfieldSource==useTerrainFromPNG:
  terrainShape = p.createCollisionShape(shapeType = p.GEOM_HEIGHTFIELD, meshScale=[.1,.1,24],fileName = "..//heightmaps/terrain1.png")
  textureId = p.loadTexture("..//heightmaps/Textures.png")
  terrain  = p.createMultiBody(0, terrainShape)
  p.changeVisualShape(terrain, -1, textureUniqueId = textureId)
...
while (p.isConnected()):
  ...
  if updateHeightfield and heightfieldSource==useProgrammatic:
    for j in range (int(numHeightfieldColumns/2)):
      for i in range (int(numHeightfieldRows/2) ):
        height = random.uniform(0.2,heightPerturbationRange)#+math.sin(time.time())
        heightfieldData[2*i+2*j*numHeightfieldRows]=height
        heightfieldData[2*i+1+2*j*numHeightfieldRows]=height
        heightfieldData[2*i+(2*j+1)*numHeightfieldRows]=height
        heightfieldData[2*i+1+(2*j+1)*numHeightfieldRows]=height
